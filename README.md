...
- Репозиторий очищен от персональных данных: `services.json` теперь пустой
...
- `services.json` — список сервисов (по умолчанию пустой)
- `settings.json` — настройки дашборда и админки
- `uploads/` — загруженные пользователем файлы
- `log/` — логи приложения и доступа

## Важно про приватные данные

Не коммитьте в публичный репозиторий:

- приватные ссылки на внутренние сервисы
- персональные названия/теги сервисов
- загруженные иконки/фоны из `uploads/`
- runtime-логи из `log/` и `server.log`
